...
public class EnergetixPowerMeterAdapter extends AModbusAdapter {
	...
	protected void activator() {
		adapterName = "Energetix Power Meter";		
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equals(DEVICE_EM101)) {
							version = deviceItem.get("device_version");
							if (version != null && version.equals("1.0")) {
								dev = new EcosailSCPMS16_v1_0(Integer.parseInt(device_instanceid), device_category);
							} else {
								dev = new EcosailSCPMS16(Integer.parseInt(device_instanceid), device_category);
							}
						} else if (device_category.equalsIgnoreCase(DEVICE_EM100)) {
							dev = new EcosailCMS16E(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_EM200)) {
							dev = new EcosailSCPMM12(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_PM200)) {
							List<Map<String, String>> deviceAtributes = dbExecute.getDeviceAttributes(device_instanceid);
							version = deviceItem.get("device_version");
							if(version != null && version.equals("1.0")){
								dev = new EcosailSCPMT12_v10(Integer.parseInt(device_instanceid), device_category);
							} else if(version != null && version.equals("1.3")){
								dev = new EcosailSCPMT12_v13(Integer.parseInt(device_instanceid), device_category);
							} else {
								dev = new EcosailSCPMT12(Integer.parseInt(device_instanceid), device_category);
							}
							dev.setAttributes(deviceAtributes);
							String debug = _props.getProperty("device_log");
							if ((debug != null) && (debug.equals("true"))) {
								dev.setDebugFlag(true);
							} else {
								dev.setDebugFlag(false);
							}
						} else if (device_category.equalsIgnoreCase(DEVICE_EM300_6)) {
							dev = new EcosailSCPMS6(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_ISAST_MULTI_GEM18)) {
							dev = new ISASTMultiGEM18(Integer.parseInt(device_instanceid), device_category);
						} 
