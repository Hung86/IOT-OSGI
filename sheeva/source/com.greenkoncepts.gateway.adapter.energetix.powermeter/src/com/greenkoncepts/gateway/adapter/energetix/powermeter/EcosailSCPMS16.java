...
public class EcosailSCPMS16 extends EcosailDevice {
	...
	// ----- Device Data
	public static final int MBREG_DATA_START = 44001;
	public static final int MBREG_DATA_NUM = 80;
	...
	public EcosailSCPMS16(int addr, String category) {
		super(category, addr);
	...
	public List<Map<String, String>> getRealTimeData(int dataIdx, boolean isRefesh) {
		...
		if (getStatus() == IDevice.DEVICE_STATUS_ONLINE) {
			for (int i = 0; i < CHANNEL_NUM; i++) {
				...
				item.put("kWh_" + (i + 1), vformat.format(reading[i] / 1000.0));
				item.put("kW_" + (i + 1), vformat.format(power[i] / 1000.0));
				item.put("kW_Avg_" + (i + 1), vformat.format(power_avg[i] / 1000.0));
				item.put("kW_Demand_" + (i + 1), vformat.format(power_max[i] / 1000.0));
