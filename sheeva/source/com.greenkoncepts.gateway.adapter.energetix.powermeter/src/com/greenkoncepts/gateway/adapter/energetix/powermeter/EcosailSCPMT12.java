...
public class EcosailSCPMT12 extends EcosailDevice {
	...
	public static final int OFFSET_PF_System = 70;
	...
	private int[] PF_System = { 0, 0, 0, 0 };
	

	private Logger mLogger = LoggerFactory.getLogger("EcosailSCPMT12");

	public EcosailSCPMT12(int addr, String category) {
		super(category, addr);
	}
	...
	private boolean decodingData(int idx, byte[] data, int mode) {
		...
		if (mode == DATA_MODE) {
			...
			PF_System[idx] = ModbusUtil.registerBEToSShort(data, OFFSET_DATA + 2 * OFFSET_PF_System);
			return true;
		}
		if (mode == CONFIG_MODE) {
			if (idx == 5) {
