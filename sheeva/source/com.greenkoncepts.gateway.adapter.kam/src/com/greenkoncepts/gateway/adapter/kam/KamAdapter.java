...
public class KamAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equals(DEVICE_PFMDPM)) {
							String version = deviceItem.get("device_version");
							if ("1.0".equals(version)) {
								dev = new PFM_DPM_v1(device_category, Integer.parseInt(device_instanceid));

							} else {
								dev = new PFM_DPM_v2(device_category, Integer.parseInt(device_instanceid));
							}
						}
