...
public class CircuitControllerAdapter  extends AModbusAdapter {
	public final static String DEVICE_CC_RELAYSTATUS = "2102"; 
	public final static String DEVICE_CC_RELAYSTATUS_8CH = "2103"; 	
	...
	protected void activator() {
		adapterName = "Circuit Controller";
		...
		try {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					CircuitControllerDevice dev = null;
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equals(DEVICE_CC_RELAYSTATUS)) {
							dev = new RelayStatus(Integer.parseInt(device_instanceid),device_category);
						} else if (device_category.equals(DEVICE_CC_RELAYSTATUS_8CH)) {
							dev = new RelayStatus_8ch(Integer.parseInt(device_instanceid),device_category);
						}
	...
	public boolean setNodeValue(String category, String deviceId, Object data) {
		mLogger.info("[setNodeValue] category = " + category + " , deviceId = "  + deviceId + ", data = " + data);
		boolean ok = false;
		if (category != null) {
			if (category.equals(DEVICE_CC_RELAYSTATUS) || category.equals(DEVICE_CC_RELAYSTATUS_8CH)) {
				for (AModbusDevice dev : _deviceList) {
					if (dev.modbusId() == Integer.valueOf(deviceId)) {
						ok = dev.setNodeValue(data);
						break;
					}
				}
			} 
