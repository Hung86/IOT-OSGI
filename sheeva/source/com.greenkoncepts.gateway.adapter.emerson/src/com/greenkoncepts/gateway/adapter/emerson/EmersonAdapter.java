...
public class EmersonAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equalsIgnoreCase(DEVICE_EMERSON_LIEBERTNXR_UPS)) {
							dev = new LiebertNXR(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_EMERSON_LIEBERTCRV_CRAC)) {
							dev = new LiebertCRV(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_EMERSON_LIEBERTPEX_CRAC)) {
							dev = new LiebertPEX(Integer.parseInt(device_instanceid), device_category);
						}
						if (dev != null) {
