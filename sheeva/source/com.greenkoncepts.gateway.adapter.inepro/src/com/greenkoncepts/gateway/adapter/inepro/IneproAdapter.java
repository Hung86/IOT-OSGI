...
public class IneproAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						if (device_category.equals(DEVICE_INEPRO_PRO1250D)) {
							int intScale = 10;
							try {
								if (deviceAtributes.size()> 0) {
									intScale = Integer.parseInt(deviceAtributes.get(0).get("data_scalar"));
								}
							} catch (NumberFormatException e) {
								mLogger.error("Device " + device_instanceid + " has wrong scale setting in configuration. Default is 10");
							}
							dev = new Pro1250D(Integer.parseInt(device_instanceid), device_category, intScale);
						}
