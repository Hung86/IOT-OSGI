...
public class FuncUtil {
	...
	/**
	 * Returns a <tt>String</tt> containing unsigned hexadecimal numbers as digits. The <tt>String</tt> will coontain two hex digit characters for
	...
	/**
	 * Returns a <tt>byte[]</tt> containing the given byte as unsigned hexadecimal number digits.
	 * <p/>
	 * 
	 * @param i
	...
	public static final boolean byteToCoil(byte data, int offset) {
		return ((data & offset) != 0 ? true : false);
	}// byteToCoil
	

	/**
	 * Converts the register (16-bit value) at the given index into a <tt>short</tt>. The value returned is:
	 * <p/>
	 * 
	 * <pre>
	 * <code>
	...
	/**
	 * Converts the given <tt>short</tt> into a register (2 bytes). The byte values in the register, in the order shown, are:
	 * <p/>
	 * 
	 * <pre>
	 * <code>
	...
	public static float registersToIEEE754FloatHighFirst(byte[] data, int index) {
		byte[] kwL1highReg = { data[index], data[index + 1] };
		byte[] kwL1lowReg = { data[index + 2], data[index + 3] };
		float result = ieee754RegistersToFloat(kwL1highReg, kwL1lowReg);
		return result;
	}
	
	public static float registersToIEEE754FloatLowFirst(byte[] data, int index) {
	...
	public static final double registersToDouble(byte[] bytes, int index) {
		return Double.longBitsToDouble((
	...
	/**
	 * Returns the broadcast address for the subnet of the host the code is executed on.
	 * 
	 * @return the broadcast address as <tt>InetAddress</tt>.
	 *         <p/>
	 *         public static final InetAddress getBroadcastAddress() { byte[] addr = new byte[4]; try { addr =
	 *         InetAddress.getLocalHost().getAddress(); addr[3] = -1; return getAddressFromBytes(addr); } catch (Exception ex) { ex.printStackTrace();
	 *         return null; } }//getBroadcastAddress
	 */

	/*
	 * public static final InetAddress getAddressFromBytes(byte[] addr) throws Exception { StringBuffer sbuf = new StringBuffer(); for (int i = 0; i <
	 * addr.length; i++) { if (addr[i] < 0) { sbuf.append(256 + addr[i]); } else { sbuf.append(addr[i]); } if (i < (addr.length - 1)) {
	 * sbuf.append('.'); } } //DEBUG:System.out.println(sbuf.toString()); return InetAddress.getByName(sbuf.toString()); }//getAddressFromBytes
	 */
	...
	static public class Temperature {
		public static final double convertFahrenheitToCelsius(double fahrenheit) {
			return ((fahrenheit - 32)/1.8);
		}// registerToShort
	}
	static public class RegisterMixEndian {
		public static final int registersToInt(byte[] bytes, int idx) {
			return (((bytes[idx] & 0xff) << 8) | (bytes[idx + 1] & 0xff) | ((bytes[idx + 2] & 0xff) << 24) | ((bytes[idx + 3] & 0xff) << 16));
		}// registersToInt
	}
	static public class RegisterLittleEndian {
		public static final short registerToShort(byte[] bytes, int idx) {
			return (short) ((bytes[idx + 1] << 8) | (bytes[idx] & 0xff));
		}// registerToShort
		
		public static final int registerToUnsignedShort(byte[] bytes, int idx) {
			return ((bytes[idx + 1] & 0xff) << 8 | (bytes[idx] & 0xff));
		}// registerToUnsignedShort
	}

	static public class RegisterBigEndian {
		/**
		 * Converts the given register (16-bit value) into a <tt>short</tt>. The value returned is:
		 * <p/>
		 * 
		 * <pre>
		 * <code>
		...
		public static final short registerToShort(byte[] bytes, int idx) {
			return  (short) ((((short)bytes[idx]) << 8) | (((short)bytes[idx + 1] ) & 0xff));
		}// registerToShort
		 ...
		 * </pre>
		 * <p/>
		 * This conversion has been taken from the documentation of the <tt>DataOutput</tt> interface.
		 * 
		 * @param v
		...
		public static final long registersToUint(byte[] bytes, int idx) {
			return ((((long) (bytes[idx] & 0xff)) << 24) |
					(((long) (bytes[idx+1] & 0xff)) << 16) |
					(((long) (bytes[idx+2] & 0xff)) << 8) |
					((long) (bytes[idx+3] & 0xff)));
		}// registersToUint

		/**
		 * Converts a byte[6] binary int value to a primitive long.<br>
		 * The value returned is:
		 * <p>
		 * 
		 * <pre>
		 ...
		 * 
		 * @param bytes
		 *            registers as <tt>byte[6]</tt>.
		 * @return the long contained in the given register bytes.
		 */
		public static final long registersToInt6(byte[] bytes) {
		 ...
		 * @return the long contained in the given register bytes.
		 */
		public static final long registersToInt6(byte[] bytes, int idx) {
			return ((((long) (bytes[idx] & 0xff)) << 40) |
					(((long) (bytes[idx + 1] & 0xff)) << 32) |
					(((long) (bytes[idx + 2] & 0xff)) << 24) |
					(((long) (bytes[idx + 3] & 0xff)) << 16) |
					(((long) (bytes[idx + 4] & 0xff)) << 8) | (((long) (bytes[idx + 5] & 0xff))));
		}// registersToInt6
		...
		public static final long registersToLong(byte[] bytes, int idx) {
