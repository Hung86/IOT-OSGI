...
public class SiemensAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equals(DEVICE_SIEMENS_SITRANS_FUE950)) {
							dev = new SitransFue950(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equals(DEVICE_SIEMENS_SITRANS_MAG6000)) {
							dev = new SitransMag6000(Integer.parseInt(device_instanceid), device_category);
						}
