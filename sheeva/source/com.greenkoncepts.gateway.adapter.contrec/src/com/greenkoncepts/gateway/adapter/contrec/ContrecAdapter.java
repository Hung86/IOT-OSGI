...
public class ContrecAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equalsIgnoreCase(DEVICE_BTU_CONTREC_MODEL212)) {
							dev = new Model212(Integer.parseInt(device_instanceid), device_category);
						}
						if (dev != null) {
