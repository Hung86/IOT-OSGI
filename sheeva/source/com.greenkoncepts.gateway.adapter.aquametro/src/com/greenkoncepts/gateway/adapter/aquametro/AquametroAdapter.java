...
public class AquametroAdapter extends AModbusAdapter {
	...
	protected void activator() {
		adapterName = "Aquatro";
		System.out.println("...waiting instance of database or tranpsort : dbService = " + dbService);
	...
	private void initialize() {
		try {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equalsIgnoreCase(DEVICE_AQUAMETRO_AMTRONMAG)) {
							dev = new AmtronMag(device_category, Integer.parseInt(device_instanceid));
						} else if (device_category.equalsIgnoreCase(DEVICE_AQUAMETRO_CALECST2)) {
							dev = new CalecSTII(device_category, Integer.parseInt(device_instanceid));
						}
			...
			try {
				...
				mLogger.error("RejectedExecutionException" , e);
