...
public class BaylanAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equalsIgnoreCase(DEVICE_WATER_BAYLAN_MODBUS_MODULE)) {
							dev = new ModbusModule(Integer.parseInt(device_instanceid), device_category);
						}
						if (dev != null) {
