...
public class DentAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equalsIgnoreCase(DEVICE_PM100_3)
								|| device_category.equalsIgnoreCase(DEVICE_PM100_18)) {
							dev = new PowerScout3(device_category, Integer.parseInt(device_instanceid));
						} else if (device_category.equalsIgnoreCase(DEVICE_PM100_3p)) {
							dev = new PowerScout3plus(device_category, Integer.parseInt(device_instanceid));
						}
