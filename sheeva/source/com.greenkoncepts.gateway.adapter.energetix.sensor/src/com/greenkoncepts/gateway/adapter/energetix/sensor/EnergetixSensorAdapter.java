...
public class EnergetixSensorAdapter extends AModbusAdapter {
			...
			if (adapterSettings.size() > 0) {
				...
				if (_numDevices > 0) {
					...
					for (int i = 0; i < _numDevices; i++) {
						...
						dev = null;
						if (device_category.equalsIgnoreCase(DEVICE_TSA01)) {
							version = deviceItem.get("device_version");
							if (version != null && version.equals("1.0")) {
								dev = new Mbsensor16TH(Integer.parseInt(device_instanceid), device_category, (float) 1.0);
							} else {
								dev = new Mbsensor16TH(Integer.parseInt(device_instanceid), device_category, (float) 2.0);
							}
						} else if (device_category.equalsIgnoreCase(DEVICE_TST01)) {
							dev = new Mbsensor16T(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_TSA02)) {
							dev = new Mbsensor1TH(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_TST02)) {
							dev = new Mbsensor1T(Integer.parseInt(device_instanceid), device_category);
						} else if (device_category.equalsIgnoreCase(DEVICE_LS01)) {
							dev = new MbsensorLux(Integer.parseInt(device_instanceid), device_category);
						}
